    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (vector != null && vector.size() > 0) {
            for (UserInfoBean userInfoBean : vector) {
                if (userInfoBean.id != null && userInfoBean.id.equals(id)) {
                    isHadRegisterInfo = true;
                    break;
                }
            }
        }
        username = AccessDB.checkUser(id, password);

        if(username == ""){
            try {
                PrintWriter out = resp.getWriter();
                out.println("<script>alert('密码错误或身份不存在');" +
        ...
        try {
            ...
            System.out.println("jump main successful...  login:"+username);
            //写入文件
